package com.confect1on.sentinel.tos;
    ...
    private void initializeTosVersion() {
        ...
        if (existingContent == null) {
            ...
            if (!config.content.isBlank() && !config.content.equals(existingContent)) {
                logger.info("ℹ️ ToS version {} exists but content differs from config - keeping database version", config.version);
                logger.info("ℹ️ To update content, change the version number in config");
            }
        }
    }
